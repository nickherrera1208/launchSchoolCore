...
function findFibonacciIndexByLength(numberOfDigits) {
  ...
  let counter = fibonacciSequence[fibonacciSequence.length - 1];
  ...
  while (counter < numberOfDigits) {
    ...
    counter = String(fibonacciSequence[fibonacciSequence.length - 1]).length;
  }
  return (`The index of the fibonacci sequence that has ${numberOfDigits} digits is ${fibonacciSequence.length - 1}, making it the 45th fibonacci
  number. The number is ${fibonacciSequence[fibonacciSequence.length - 1]}`);
}
